...
pub fn handler(ctx: Context<InjectToRoot>, is_mutable: bool, bump: u8) -> Result<()> {
    ...
    let (_, children_pda_bump) =
        Pubkey::find_program_address(&[&CHILDREN_PDA_SEED[..], parent_key, child_key], &(ctx.program_id));
